...
type EsignStatus = "INITIATED" | "AWAITING_ESP" | "PROCESSING" | "COMPLETED" | "FAILED";
...
export function EsignSignerModal({
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data?.type === "ESIGN_CALLBACK" && event.data?.aspTxnId === aspTxnId) {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      if (!aspTxnId || esignStatus === "COMPLETED" || esignStatus === "FAILED" || esignStatus === "INITIATED") return;
      try {
  ...
  const handleInitiate = async () => {
    ...
    try {
      ...
      const popup = window.open("", "EsignPopup", "width=800,height=600,status=yes,scrollbars=yes");
      if (popup) {
        ...
        throw new Error("Popup blocked. Please allow popups for this site to sign.");
      }
    } catch (err: any) {
      setEsignStatus("FAILED");
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => {
      ...
      if ((esignStatus === "AWAITING_ESP" || esignStatus === "PROCESSING") && !val) return;
      onOpenChange(val);
    }}>
      <DialogContent className="sm:max-w-4xl max-h-[90vh] flex flex-col p-6">
        <DialogHeader>
          ...
          <DialogDescription>
            Review the document and provide legal consent to cryptographically sign via CDAC/NSDL.
          </DialogDescription>
        </DialogHeader>
        
        <div className="flex-1 flex flex-col md:flex-row gap-6 min-h-[500px] mt-4">
          <div className="flex-1 rounded-md border bg-muted overflow-hidden">
            <iframe 
              src={esignStatus === "COMPLETED" && signedPdfUrl ? signedPdfUrl : unsignedPdfUrl} 
          ...
          <div className="w-full md:w-[320px] flex flex-col gap-6 rounded-md border bg-card p-5 shadow-sm">
              ...
              <>
                <div className="flex flex-col gap-3 rounded-lg border border-primary/20 bg-primary/5 p-4 text-sm">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="leading-relaxed">
                      By proceeding, I authorize the eSign Application Service Provider (ASP) to fetch and embed my Aadhaar-linked cryptographic signature.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 space-y-0 pt-2">
                  ...
                  <Label htmlFor="consent" className="text-sm font-medium leading-tight cursor-pointer">
                    I agree to eSign this document using Aadhaar (ID: {signerAadhaarMasked}).
                  </Label>
                </div>
              ...
              <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
                ...
                <div className="space-y-1">
                  <h3 className="font-semibold text-lg">
                    {esignStatus === "AWAITING_ESP" ? "Awaiting Signature" : "Processing Signature"}
                  </h3>
            ...
            {esignStatus === "COMPLETED" && (
              <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
                ...
                <div className="space-y-1">
                  <h3 className="font-semibold text-lg text-green-700">Signature Applied</h3>
                  <p className="text-sm text-muted-foreground">
            ...
            {esignStatus === "FAILED" && (
              <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
                ...
                <Button className="w-full mt-4" onClick={() => setEsignStatus("INITIATED")}>
                  Try Again
                </Button>
              </div>
            )}
